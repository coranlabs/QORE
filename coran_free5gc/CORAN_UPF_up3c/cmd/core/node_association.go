	...
	for {
		...
		case <-time.After(time.Duration(config.Conf.HeartbeatTimeout) * time.Second):
			if !association.HandleHeartbeatTimeout() {
				logger.Pfcplog.Warnf("the number of unanswered heartbeats has reached the limit, association deleted: %s", association.Addr)
		...
		case <-ctx.Done():
			logger.MainLog.Infof("HeartbeatScheduler context done | association address: %s", association.Addr)
			return
		}
	}
}
