...
func (bpfObjects *BpfObjects) Load() error {
	...
	if err := os.MkdirAll(pinPath, os.ModePerm); err != nil {
		logger.MainLog.Infof("failed to create bpf fs subpath: %+v", err)
		return err
	}
...
func ResizeEbpfMap(eMap **ebpf.Map, eProg *ebpf.Program, newSize uint32) error {
	...
	if err != nil {
		logger.MainLog.Infof("Failed get ebpf map info: %s", err)
		return err
	}
	...
	if err != nil {
		logger.MainLog.Infof("Failed to close old ebpf map: %s, %+v", err, *eMap)
		return err
	}
	...
	err = (*eMap).Unpin()
	if err != nil {
		logger.MainLog.Infof("Failed to unpin old ebpf map: %s, %+v", err, *eMap)
		return err
	}
	...
	*eMap, err = ebpf.NewMapWithOptions(mapSpec, ebpf.MapOptions{})
	if err != nil {
		logger.MainLog.Infof("Failed to create resized ebpf map: %s", err)
		return err
	}
	err = eProg.BindMap(*eMap)
	if err != nil {
		logger.MainLog.Infof("Failed to bind resized ebpf map: %s", err)
		return err
	}
	return nil
}
