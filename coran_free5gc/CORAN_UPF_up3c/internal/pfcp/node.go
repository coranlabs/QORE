...
import (
	...
	"github.com/coranlabs/HEXA_UPF/internal/forwarder"
...
func (s *Sess) CreatePDR(req *ie.IE) error {
	...
	for _, i := range ies {
		...
		case ie.PDRID:
			...
			pdrid = v
		case ie.PDI:
			// s.log.Println("in pdi broooo")
			v, err := i.PDI()

			if err != nil {
				logger.MainLog.Tracef("error %v", err)
				return err
			} else {
				// logger.MainLog.Tracef("in pdi broooo %v", v)
			}
			...
			for _, g := range v {
				...
				case ie.FTEID:
					f, err := g.FTEID()
					if err != nil {
						logger.MainLog.Tracef("fteid err %v", err)
					} else {
						logger.MainLog.Tracef("fteid is %v", f)
					}
				}
			}
			// v, err1 := i.PDRID()
