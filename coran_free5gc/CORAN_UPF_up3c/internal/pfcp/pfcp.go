...
func NewPfcpServer(cfg *factory.Config, driver forwarder.Driver) *PfcpServer {
	...
	NodeID := net.ParseIP(os.ExpandEnv(cfg.Pfcp.NodeID))
	return &PfcpServer{
		cfg:          cfg,
		listen:       cfg.Pfcp.Addr,
...
func (s *PfcpServer) main(wg *sync.WaitGroup) {
	...
	var err error
	ip := net.ParseIP(os.ExpandEnv(s.listen))
	addr := fmt.Sprintf("%s:%d", ip.String(), factory.UpfPfcpDefaultPort)
