...
import (
	...
	"github.com/coranlabs/CORAN_CHF/Messages_handling_entity/cdr/asn"
	"github.com/coranlabs/CORAN_CHF/Messages_handling_entity/cdr/cdrFile"
	"github.com/coranlabs/CORAN_CHF/Messages_handling_entity/cdr/cdrType"
	"github.com/coranlabs/CORAN_CONSOLE/backend/console_context"
...
func GetChargingRecord(c *gin.Context) {
	...
	webuiSelf := console_context.GetSelf()
	...
	if amfUris := webuiSelf.GetOamUris(models.NfType_AMF); amfUris != nil {
		...
		ctx, pd, tokerErr := console_context.GetSelf().GetTokenCtx(models.ServiceName_NAMF_OAM, models.NfType_AMF)
		if tokerErr != nil {
		...
		if err = console_context.GetSelf().RequestBindToken(req, ctx); err != nil {
			logger.ProcLog.Error(err)
			c.JSON(http.StatusInternalServerError, gin.H{})
			return
		}
