...
type UpfConfig struct {
	...
	MaxRetrans        uint32        `mapstructure:"maxretrans" validate:"min=1" json:"maxretrans"`         // New Field
	RetransTimeout    time.Duration `mapstructure:"retranstimeout" validate:"min=1" json:"retranstimeout"` // New Field
}

func init() {
	var configPath = pflag.String("config", "./hexa.yml", "Path to config file")
	logger.Initialize(0)
	...
	_ = v.BindPFlag("teid_pool", pflag.Lookup("teidpool"))
	_ = v.BindPFlag("Gatewayip", pflag.Lookup("Gatewayip"))
	_ = v.BindPFlag("maxretrans", pflag.Lookup("maxretrans"))
	_ = v.BindPFlag("retranstimeout", pflag.Lookup("retranstimeout"))

	v.SetDefault("interface_name", "eth0")
	v.SetDefault("xdp_attach_mode", "generic")
	v.SetDefault("api_address", ":8080")
	v.SetDefault("pfcp_address", "10.100.200.14:8805")
	v.SetDefault("pfcp_node_id", "10.100.200.14")
	v.SetDefault("metrics_address", ":9090")
	v.SetDefault("n3_address", "10.100.200.14")
	...
	v.SetDefault("Gatewayip", "10.100.50.236")
	v.SetDefault("retranstimeout", 1000)
	v.SetDefault("maxretrans", 5)
	...
	if err := v.ReadInConfig(); err != nil {
		if _, ok := err.(viper.ConfigFileNotFoundError); ok {
			...
			logger.CfgLog.Errorf("Unable to read config file: %v. Using defaults", err)
		}
	}
	logger.CfgLog.Debugf("Startup config: %+v", v.AllSettings())
}
