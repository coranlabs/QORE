...
type SFarInfo struct {
	FarInfo  ebpf_datapath.FarInfo
	GlobalId uint32
}

type SQerInfo struct {
	QerInfo  ebpf_datapath.QerInfo
	GlobalId uint32
}

func (s *Session) NewFar(id uint32, internalId uint32, farInfo ebpf_datapath.FarInfo) {
	s.FARs[id] = SFarInfo{
		FarInfo:  farInfo,
		GlobalId: internalId,
	}
...
func (s *Session) UpdateFar(id uint32, farInfo ebpf_datapath.FarInfo) {
	sFarInfo := s.FARs[id]
...
func (s *Session) NewQer(id uint32, internalId uint32, qerInfo ebpf_datapath.QerInfo) {
	s.QERs[id] = SQerInfo{
		QerInfo:  qerInfo,
		GlobalId: internalId,
	}
...
func (s *Session) UpdateQer(id uint32, qerInfo ebpf_datapath.QerInfo) {
	sQerInfo := s.QERs[id]
