...
import (
	...
	pfcp_message "github.com/coranlabs/CORAN_SMF/Messages_handling_entity/pfcp/message"
)
...
func modifyExistingPfcpSession(
	...
	rcvMsg, err := pfcp_message.SendPfcpSessionModificationRequest(
		state.upf, smContext, state.pdrList, state.farList, state.barList, state.qerList, state.urrList, 0)
	if err != nil {
...
func (p *Processor) updateAnUpfPfcpSession(
	...
	rcvMsg, err := pfcp_message.SendPfcpSessionModificationRequest(
		ANUPF.UPF, smContext, pdrList, farList, barList, qerList, urrList, SEMP)
	if err != nil {
