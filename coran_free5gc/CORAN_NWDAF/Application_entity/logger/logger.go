...
var (
	...
	SBILog        *log.Entry
	EventLog      *log.Entry
	SubscriberLog *log.Entry
	EngineLog     *log.Entry
	AnalyticsLog  *log.Entry
)
...
func (f *ColorFormatter) Format(entry *log.Entry) ([]byte, error) {
	...
	coloredLevel := fmt.Sprintf("%s[ %-4s ]%s", color, strings.ToUpper(entry.Level.String()), colorReset)
	...
	coloredComponent := fmt.Sprintf("%s[ %-6s ]%s", color, strings.ToUpper(component.(string)), colorReset)
...
func InitializeLogger(level log.Level) {
	...
	EventLog = CreateLoggerWithComponent("EVENT")
	SubscriberLog = CreateLoggerWithComponent("SUBS")
	EngineLog = CreateLoggerWithComponent("ENGN")
	AnalyticsLog = CreateLoggerWithComponent("ANALYT")
}
