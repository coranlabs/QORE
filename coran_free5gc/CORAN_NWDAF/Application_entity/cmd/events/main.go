...
func main() {
	...
	// Initialize internal events package
	events.InitConfig()
	interfaceName := "eth0"

	// Create routers for events
	...
	ip += ":8000"
	// Start server for events
	startServer(ip, eventsRouter)
}
...
func startServer(addr string, handler http.Handler) {
	...
	logger.EventLog.Infof("Events Server listening at %s", addr)
