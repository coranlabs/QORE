...
func farToCreateFAR(far *context.FAR) *pfcp.CreateFAR {
	...
	if far.ForwardingParameters != nil {
		...
		logger.AppLog.Infof("DestinationInterface : %d", far.ForwardingParameters.DestinationInterface)
	...
	for _, pdr := range pdrList {
		if pdr == pdrList[0] {
			...
			case context.RULE_INITIAL:
				logger.AppLog.Infof("initial BuildPfcpSessionModificationRequest")
				msg.CreatePDR = append(msg.CreatePDR, pdrToCreatePDR(pdr))
			case context.RULE_UPDATE:
				logger.AppLog.Infof("update BuildPfcpSessionModificationRequest")
			...
			logger.AppLog.Infof("it was uplink pdr not downlink value")
		}
	}

	for _, far := range farList {
		if far == farList[0] {
			far.State = context.RULE_INITIAL
			switch far.State {
			case context.RULE_INITIAL:
				msg.CreateFAR = append(msg.CreateFAR, farToCreateFAR(far))
			case context.RULE_UPDATE:
			...
			logger.AppLog.Infof("it was uplink far not downlink value")
		}
	}
