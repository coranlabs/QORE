...
func HandlePfcpAssociationSetupResponse(msg *pfcpUdp.Message) {
	...
	if rsp.Cause.CauseValue == pfcpType.CauseRequestAccepted {
		...
		if rsp.UserPlaneIPResourceInformation != nil {
			rsp.UserPlaneIPResourceInformation.NetworkInstance = util_3gpp.Dnn("hexa")
		...
		if rsp.UserPlaneIPResourceInformation == nil {
			...
			n3Interface.NetworkInstance = string("hexa")
			...
			logger.PfcpLog.Infof("UPF(%s)[%s] setup association success N3(%s)", upf.NodeID.ResolveNodeIdToIp().String(), upf.UPIPInfo.NetworkInstance, upf.UPIPInfo.Ipv4Address.String())
		} else {
