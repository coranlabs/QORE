...
func printAssociationSetupRequest(req *message.AssociationSetupRequest) {
	...
	logger.PFCPSessLog.Infof("+---------------------------------------------+")
	logger.PFCPSessLog.Infof("|         Association Setup Request           |")
	logger.PFCPSessLog.Infof("+---------------------------------------------+")

	nodeId, err := req.NodeID.NodeID()
	if err == nil {
		// writeLineTabbed(&sb, fmt.Sprintf("Node ID: %s", nodeId), 1)
		logger.PFCPSessLog.Infof("| Node ID:      |%-29s|", nodeId)
		// logger.PFCPSessLog.Infof("|Node ID: %s", nodeId)
	}
	if req.RecoveryTimeStamp != nil {
		...
		if err == nil {
			...
			// logger.PFCPSessLog.Infof("| Recovery Time: %s", recoveryTime.String())
			logger.PFCPSessLog.Infof("| Recovery Time:|%-29s|", recoveryTime.String())
			logger.PFCPSessLog.Infof("+---------------------------------------------+")
		}
	}
...
func printSessionEstablishmentRequest(req *message.SessionEstablishmentRequest) {
	...
	logger.PFCPSessLog.Infof("+---------------------------------------------+")
	logger.PFCPSessLog.Infof("|       Session Establishment Request         |")
	logger.PFCPSessLog.Infof("+---------------------------------------------+")
	logger.PFCPSessLog.Infof("|                     PDR                     |")
	logger.PFCPSessLog.Infof("+---------------------------------------------+")
	for _, pdr := range req.CreatePDR {
		displayPdr(&sb, pdr)
	}

	logger.PFCPSessLog.Infof("|                     FAR                     |")
	logger.PFCPSessLog.Infof("+---------------------------------------------+")
	for _, far := range req.CreateFAR {
		displayFar(&sb, far)
	}

	logger.PFCPSessLog.Infof("|                     QER                     |")
	logger.PFCPSessLog.Infof("+---------------------------------------------+")
...
func printSessionModificationRequest(req *message.SessionModificationRequest) {
	...
	logger.PFCPSessLog.Infof("+---------------------------------------------+")
	logger.PFCPSessLog.Infof("|        Session Modification Request         |")
	logger.PFCPSessLog.Infof("+---------------------------------------------+")
	logger.PFCPSessLog.Infof("|                     PDR                     |")
	logger.PFCPSessLog.Infof("+---------------------------------------------+")
	for _, pdr := range req.CreatePDR {
...
func displayFar(sb *strings.Builder, far *ie.IE) {
	...
	// logger.PFCPSessLog.Infof("+----------------------+----------------------+")
	logger.PFCPSessLog.Infof("|%-45s|", "Create FAR")
	logger.PFCPSessLog.Infof("|%-22s| %-21d|", "  FAR ID ", farId)

	applyAction, err := far.ApplyAction()
	if err == nil {
		// writeLineTabbed(sb, fmt.Sprintf("Apply Action: %+v ", applyAction), 2)
		logger.PFCPSessLog.Infof("|%-22s| %-19v|", "  TEID ", applyAction)

	}
	if forwardingParameters, err := far.ForwardingParameters(); err == nil {
		// writeLineTabbed(sb, "Forwarding Parameters:", 2)
		// logger.PFCPSessLog.Infof("forwarding parameters : %v", forwardingParameters)
		for _, forwardingParameter := range forwardingParameters {
			...
			if err == nil {
				...
				logger.PFCPSessLog.Infof("|%-22s| %-21s|", "  network instance ", networkInstance)
			...
			if err == nil {
				...
				logger.PFCPSessLog.Infof("Outer Header Creation: %+v ", outerHeaderCreation)
			}
	...
	if updateForwardingParameters, err := far.UpdateForwardingParameters(); err == nil {
		...
		for _, updateForwardingParameter := range updateForwardingParameters {
			...
			if err == nil {
				// logger.PFCPSessLog.Infof("Update forwarding Parameters:")
				// writeLineTabbed(sb, fmt.Sprintf("Network Instance: %s ", networkInstance), 3)
				// logger.PFCPSessLog.Infof("Network Instance: %s ", networkInstance)
				logger.PFCPSessLog.Infof("|%-22s| %-21s|", "  network instance ", networkInstance)
			}
...
func displayPdr(sb *strings.Builder, pdr *ie.IE) {
	...
	// logger.PFCPSessLog.Infof("+----------------------+----------------------+")
	logger.PFCPSessLog.Infof("|%-45s|", "Create PDR")
	logger.PFCPSessLog.Infof("|%-22s| %-21d|", "  PDR ID ", pdrId)
	...
	if outerHeaderRemoval, err := pdr.OuterHeaderRemovalDescription(); err == nil {
		...
		logger.PFCPSessLog.Infof("|%-20s| %-21d|", "  outer header removal", outerHeaderRemoval)
	}
	...
	if pdi, err := pdr.PDI(); err == nil {
		...
		logger.PFCPSessLog.Infof("|%-22s| %-21d|", "  source interface ", srcInterface)
		...
		if teidPdiId := findIEindex(pdi, 21); teidPdiId != -1 { // IE Type F-TEID
			if fteid, err := pdi[teidPdiId].FTEID(); err == nil {
				...
				logger.PFCPSessLog.Infof("|%-22s| %-21d|", "  TEID ", fteid.TEID)
				// writeLineTabbed(sb, fmt.Sprintf("Ipv4: %+v ", fteid.IPv4Address), 2)
				logger.PFCPSessLog.Infof("|%-22s| %-21v|", "  IPv4 ", fteid.IPv4Address)
				// writeLineTabbed(sb, fmt.Sprintf("Ipv6: %+v ", fteid.IPv6Address), 2)
				logger.PFCPSessLog.Infof("|%-22s| %-21v|", "  IPv6 ", fteid.IPv6Address)
			}
		}
		...
		if ueipPdiId := findIEindex(pdi, 93); ueipPdiId != -1 { // IE Type UE IP Address
			if ueIp, _ := pdi[ueipPdiId].UEIPAddress(); ueIp != nil {
				if ueIp.IPv4Address != nil {
					...
					logger.PFCPSessLog.Infof("|%-22s| %-21v|", "  UE IPv4 Address", ueIp.IPv4Address)
				}
				if ueIp.IPv6Address != nil {
					writeLineTabbed(sb, fmt.Sprintf("UE IPv6 Address: %s ", ueIp.IPv6Address), 2)
				}
			} else {
				logger.PFCPSessLog.Infof("ueIp is nil. ueipPdiId: %d", ueipPdiId)
			}
		}
