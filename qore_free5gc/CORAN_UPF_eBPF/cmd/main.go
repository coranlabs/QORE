...
import (
	...
	"github.com/coranlabs/CORAN_UPF_eBPF/cmd/core/service"
	config "github.com/coranlabs/CORAN_UPF_eBPF/config"
	"github.com/coranlabs/CORAN_UPF_eBPF/logger"
	"github.com/gin-gonic/gin"

	"github.com/coranlabs/CORAN_UPF_eBPF/cmd/api/rest"
	"github.com/coranlabs/CORAN_UPF_eBPF/cmd/server"
...
func Emain() {
	...
	if err := ebpf.IncreaseResourceLimits(); err != nil {
		logger.InitLog.Errorf("Can't increase resource limits: %s", err.Error())
	}

	bpfObjects := ebpf.NewBpfObjects()
	if err := bpfObjects.Load(); err != nil {
		logger.InitLog.Errorf("Loading bpf objects failed: %s", err.Error())
	}
	...
	if config.Conf.EbpfMapResize {
		if err := bpfObjects.ResizeAllMaps(config.Conf.QerMapSize, config.Conf.FarMapSize, config.Conf.PdrMapSize); err != nil {
			logger.InitLog.Errorf("Failed to set ebpf map sizes: %s", err)
		}
	}
	...
	for _, ifaceName := range config.Conf.InterfaceName {
		...
		if err != nil {
			logger.InitLog.Errorf("Lookup network iface %q: %s", ifaceName, err.Error())
		}
		...
		if err != nil {
			logger.InitLog.Errorf("Could not attach XDP program: %s", err.Error())
		}
		defer l.Close()
	...
	if err != nil {
		logger.InitLog.Errorf("Could not create PFCP connection: %s", err.Error())
	}
	go pfcpConn.Run()
	...
	go func() {
		if err := apiSrv.Run(); err != nil {
			logger.InitLog.Errorf("Could not start api server: %s", err.Error())
		}
	}()
	...
	go func() {
		if err := metricsSrv.Run(); err != nil {
			logger.InitLog.Errorf("Could not start metrics server: %s", err.Error())
		}
	}()
	...
	for {
		...
		case <-ticker.C:
			...
			// if err != nil {
			// 	logger.InitLog.Tracef("Error reading map: %s", err)
			// 	continue
			// }
			// logger.InitLog.Tracef("Pipeline map contents:\n%s", s)
		case <-stopper:
			logger.InitLog.Infof("Received signal, exiting program..")
			return
		}
	}
}
