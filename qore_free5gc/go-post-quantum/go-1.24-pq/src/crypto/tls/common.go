...
const (
	...
	// MLDSA65         SignatureScheme = 0xfe63
	// Ed448Dilithium3 SignatureScheme = 0xfe62
)
...
func (c *Config) Clone() *Config {
	...
	/*
		Added PQSignatureSchemesEnabled to Config struct because without copying your custom field, the clone loses that data, and downstream code behaves as if it's false or unset.
	*/
}
...
func (chi *ClientHelloInfo) SupportsCertificate(c *Certificate) error {
	...
	if priv, ok := c.PrivateKey.(crypto.Signer); ok {
		...
		case circlSign.PublicKey:
			scheme := pub.Scheme()
			_, ok := scheme.(circlPki.TLSScheme)
			if !ok {
				return errors.New("no TLS Identity Scheme found")
			}
			return nil
		case *rsa.PublicKey:
		default:
...
const (
	...
	// Asymmetric Encryption keys (for test purposes only)
	keyLogLabelClientECDH       = "CLIENT_ECDH_PUBLIC_KEY"
	keyLogLabelClientKEM        = "CLIENT_KEM_PUBLIC_KEY"
	keyLogLabelCipherTextKEM    = "KEM_ENC_CIPHERTEXT"
	keyLogLabelSharedSecretECDH = "ECDH_SHARED_SECRET"
	keyLogLabelSharedSecretKEM  = "KEM_SHARED_SECRET"

	keyLogLabelCertVerifySig = "CERTIFICATE_VERIFY"
)
