...
func signingParamsForKey(pub any, sigAlgo SignatureAlgorithm) (SignatureAlgorithm, pkix.AlgorithmIdentifier, error) {
	var ai pkix.AlgorithmIdentifier
	var pubType PublicKeyAlgorithm
	var defaultAlgo SignatureAlgorithm

	switch pub := pub.(type) { // key.Public() interoperable with circl sign.PublicKey
	...
	case circlSign.PublicKey:
		...
		defaultAlgo = SignatureAlgorithmByCirclScheme(scheme) // ex: PureMLDSA65

	default:
		// if pubKey, ok := pub.(circlSign.PublicKey); ok {
...
func CreateCertificate(rand io.Reader, template, parent *Certificate, pub, priv any) ([]byte, error) {
	...
	signatureAlgorithm, algorithmIdentifier, err := signingParamsForKey(pub, template.SignatureAlgorithm)
	if err != nil {
		return nil, err
	}
...
func (c *Certificate) CreateCRL(rand io.Reader, priv any, revokedCerts []pkix.RevokedCertificate, now, expiry time.Time) (crlBytes []byte, err error) {
	...
	signatureAlgorithm, algorithmIdentifier, err := signingParamsForKey(key.Public(), 0)
	if err != nil {
		return nil, err
	}
...
func CreateCertificateRequest(rand io.Reader, template *CertificateRequest, priv any) (csr []byte, err error) {
	...
	signatureAlgorithm, algorithmIdentifier, err := signingParamsForKey(key.Public(), template.SignatureAlgorithm)
	if err != nil {
		return nil, err
	}
...
func CreateRevocationList(rand io.Reader, template *RevocationList, issuer *Certificate, priv crypto.Signer) ([]byte, error) {
	...
	signatureAlgorithm, algorithmIdentifier, err := signingParamsForKey(priv.Public(), template.SignatureAlgorithm)
	if err != nil {
		return nil, err
	}
