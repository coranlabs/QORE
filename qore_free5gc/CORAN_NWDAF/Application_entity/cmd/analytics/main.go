...
func main() {
	...
	// Initialize internal analytics package
	interfaceName := "eth0"
	...
	if err != nil {
		logger.MainLog.Errorf("Failed to get IP address from interface %s: %v", interfaceName, err)
	}

	ip += ":8000"

	// Start server for analytics
	startServer(ip, analyticsRouter)
}
...
func startServer(addr string, handler http.Handler) {
	...
	logger.AnalyticsLog.Infof("Analytics Server listening at %s", addr)
