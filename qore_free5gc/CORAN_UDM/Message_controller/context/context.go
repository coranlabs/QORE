...
func InitUdmContext(context *UDMContext) {
	...
	logger.AppLog.Info("udmconfig Info: Version[", config.Info.Version, "] Description[", config.Info.Description, "]")
	...
	if sbi != nil {
		...
		if udmContext.BindingIPv4 != "" {
			logger.AppLog.Info("Parsing ServerIPv4 address from ENV Variable.")
		} else {
			udmContext.BindingIPv4 = sbi.BindingIPv4
			if udmContext.BindingIPv4 == "" {
				logger.AppLog.Warn("Error parsing ServerIPv4 address as string. Using the 0.0.0.0 address as default.")
