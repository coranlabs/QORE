...
import (
	...
	"github.com/coranlabs/HEXA_UPF/internal/emp"
	"github.com/coranlabs/HEXA_UPF/internal/report"
)

var source_fteid *FTEIDFields
var target_fteid *FTEIDFields
...
func (s *PfcpServer) handleSessionEstablishmentRequest(
	...
	for _, i := range req.CreatePDR {
		...
		if err2 != nil {
			sess.log.Tracef("Est fteid error: %+v", err2)
		}
		source_fteid = (*FTEIDFields)(f)
	}

	var v4 net.IP
	ip := net.ParseIP(os.ExpandEnv(s.nodeID))
	addrv4, err := net.ResolveIPAddr("ip4", ip.String())
	if err == nil {
		v4 = addrv4.IP.To4()
	}
	// TODO: support v6
...
func (s *PfcpServer) handleSessionModificationRequest(
	...
	for _, i := range req.CreateFAR {
		...
		if err2 != nil {
			sess.log.Tracef("Mod fteid error: %+v", err2)
		}
		target_fteid = (*FTEIDFields)(f)
	}
	...
	if req.SEMP == 1 {
		emp.Send_emp(source_fteid.TEID, source_fteid.IPv4Address)
		source_fteid = target_fteid
	}
