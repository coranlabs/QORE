...
func init() {
	var configPath = pflag.String("config", "./conf/hexa.yml", "Path to config file")
	...
	if err := v.ReadInConfig(); err != nil {
		if _, ok := err.(viper.ConfigFileNotFoundError); ok {
			...
			logger.MainLog.Tracef("Unable to read config file: %v. Using defaults", err)
		}
	}

	logger.MainLog.Tracef("Startup config: %+v", v.AllSettings())
}
