...
func CreatePfcpConnection(addr string, pfcpHandlerMap PfcpHandlerMap, nodeId string, n3Ip string, mapOperations ebpf.ForwardingPlaneController, resourceManager *service.ResourceManager) (*PfcpConnection, error) {
	...
	if err != nil {
		logger.Pfcplog.Warnf("Can't resolve UDP address: %s", err.Error())
		return nil, err
	}
	udpConn, err := net.ListenUDP("udp", udpAddr)
	if err != nil {
		logger.Pfcplog.Warnf("Can't listen UDP address: %s", err.Error())
		return nil, err
	}
	...
	logger.InitLog.Infof("Starting PFCP connection: %v with Node ID: %v and N3 address: %v", udpAddr, nodeId, n3Addr)
...
func (connection *PfcpConnection) Run() {
	...
	for {
		...
		if err != nil {
			logger.Pfcplog.Warnf("Error reading from UDP socket: %s", err.Error())
			time.Sleep(1 * time.Second)
			continue
		}
		logger.MainLog.Debugf("Received %d bytes from %s", n, addr)
...
func (connection *PfcpConnection) DeleteAssociation(assocAddr string) {
	...
	logger.MainLog.Infof("Pruning expired node association: %s", assocAddr)
	for sessionId, session := range assoc.Sessions {
		logger.MainLog.Infof("Deleting session: %d", sessionId)
